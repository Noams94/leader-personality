...
export default function NavBar({ locale, t }: NavBarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className={`flex items-center justify-between h-16 ${isRtl ? 'flex-row-reverse' : ''}`}>
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center text-white font-bold text-sm group-hover:bg-blue-600 transition-all duration-300 ease-in-out">
              LP
            </div>
          ...
          <div className={`hidden md:flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
            <button
              ...
              className="text-sm text-gray-500 hover:text-gray-900 transition-all duration-300 ease-in-out px-3 py-1.5 rounded-lg hover:bg-gray-50 border border-gray-200"
            >
              {t.language}
            </button>
            <Link
              href="/assessment"
              className="px-5 py-2 bg-blue-500 text-white rounded-lg text-sm font-semibold hover:bg-blue-600 hover:-translate-y-0.5 transition-all duration-300 ease-in-out shadow-sm"
            >
              {t.assessment}
            </Link>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 py-3 space-y-1">
            ...
            <Link
              href="/assessment"
              onClick={() => setMenuOpen(false)}
              className="block mx-4 mt-2 py-2 px-4 bg-blue-500 text-white rounded-lg text-sm font-semibold text-center"
            >
              {t.assessment}
            </Link>
          </div>
